...
router.get('/bookings', protect, authorize('superadmin', 'manager'), async (req, res, next) => {
  try {
    ...
    const rows = bookings.map((b, i) => ({
      ...
      'Room & Bed Choice': b.roomPreference?.roomType ? `${b.roomPreference.roomType} (${b.roomPreference.bedType || 'Double Bed'}, ${b.roomPreference.roomCount || 1} Room)` : 'Standard Cottage',
